...
\textit{Es un \alert{integer} que corresponde con la profundidad del mapa de características de salida.}

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{Slides/figures/Tema 3/DimensionFilters.png}
    \caption{A medida que la imagen \alert{reduce} su tamaño, los filtros \alert{aumentan}. Se pretende extraer más \alert{características de alto nivel} y cubrir zonas amplias de la imagen original.}
\end{figure}
\end{frame}
...
\section{Construcción de una Convolutional Neural Network (CNN)}

\begin{frame}{Construcción de una CNN}
La estructura de \alert{embudo} típica de las redes neuronales \alert{clasificadoras} también se aplica a CNN. Para ello el objetivo es \alert{reducir} la dimensión de la imagen hasta generar una salida.
...
Otra \alert{opción} para la reducción dimensional es el uso de convoluciones con \alert{strides} que no sean (1, 1).

\begin{itemize}
    \item Cierta literatura apunta a esta aproximación como \say{\alert{más inteligente}} ya que se le permite a la \alert{red} que sea ella la que escoja \alert{cómo hacer} la reducción.
    \item El principal \alert{inconveniente} es que usando este método se aumenta el número de parámetros de la red.
    
\end{itemize}


Existe un debate abierto sobre si es preferible que la red \alert{aprenda} a realizar el \textit{downsampling} \cite{MaxPoolingvsStrides}, o si resulta más adecuado emplear técnicas como \alert{MaxPooling} \cite{sun2018fishnet}.
...
\begin{itemize}
    \item Enfoques comunes
    \begin{itemize}
        \item \alert{Feature extraction}: congelar capas y usar sus salidas como extractor de características.
        \item \alert{Fine-tuning}: reentrenar parcialmente algunas capas junto con las nuevas.
    \end{itemize}
    \item Ventajas
    \begin{itemize}
        \item Reduce el tiempo de entrenamiento.
        \item Mejora los resultados con pocos datos.
        \item Facilita el uso de arquitecturas complejas.
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Ejemplo: VGG16}
    \centering
    \includegraphics[width=0.5\textwidth]{Slides/figures/Tema 1/image-398.png}
    
\begin{itemize}
    \item VGG16
    \begin{itemize}
        \item Red profunda propuesta por Simonyan y Zisserman (2014). \url{https://arxiv.org/abs/1409.1556}
        \item Basada en bloques de convoluciones en embudo y luego densa.
    \end{itemize}

    \item Uso en Transfer Learning
    \begin{itemize}
        \item Importar el modelo preentrenado. \alert{Congelar sus pesos}.
        \item Sustituir densa por nueva densa.
        \item Opcionalmente: descangelar algún filtro del final.
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Ejemplo/Ejercicio}
    \centering
    \includegraphics[width=0.4\textwidth]{Slides/figures/GoogleColab.png}
\begin{itemize}
    \centering
    \item {\Large \href{https://colab.research.google.com/github/guillermoih/M-todos-Generativos/blob/main/Notebooks/1.2_05-Cuadros.ipynb}{1.2\_05-Cuadros.ipynb}}
\end{itemize}
\end{frame}
