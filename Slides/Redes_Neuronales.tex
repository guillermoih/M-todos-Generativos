...
Una \alert{red neuronal} es un sistema matemático capaz de aprender a realizar predicciones a partir de unos datos de entrada, fue propuesta por McCulloch y Pitts en 1943 \cite{mcculloch1943logical}, se basaba en \alert{imitar} el comportamiento de una neurona biológica.
...
\begin{itemize}
    ...
    \item \alert{1958: Perceptrón} (Rosenblatt): Primera red neuronal capaz de aprender automáticamente ajustando sus pesos
...
\begin{frame}{Redes neuronales artificiales: Ventajas}
El paradigma del Deep Learning tiene ciertas características \alert{específicas} que definen su comportamiento y resultados.

Dentro de sus \alert{ventajas} se pueden encontrar:
\begin{itemize}
    \item Tratamiento de problemas \alert{muy complejos}.
    \item Mucha facilidad para \alert{extrapolación} a distintos casos de uso.
    \item Amplia \alert{popularidad}.
\end{itemize}
\end{frame}

\begin{frame}{Redes neuronales artificiales: Desventajas}
El paradigma del Deep Learning tiene ciertas características \alert{específicas} que definen su comportamiento y resultados.

Dentro de sus \alert{desventajas} se pueden encontrar:
\begin{itemize}
    \item Necesidad de grandes \alert{volúmenes de datos}.
    \item Muy alta \alert{complejidad} en su diseño y uso.
    \item Sistema caótico (poco determinista).
    \item No explicabilidad.
    \item Uso intensivo de recursos \alert{computacionales} y de \alert{memoria}.
\end{itemize}
\end{frame}
...
\href{https://www.deeplearning.ai/ai-notes/optimization/index.html}{Recursos visuales del descenso del gradiente}
\end{frame}
...
Los \alert{optimizadores} son \alert{modificaciones} sobre el algoritmo de descenso del gradiente para \alert{mejorar} la velocidad y convergencia del modelo.

Los más comunes son:
\begin{itemize}
    \item \alert{Learning rate decay}
    \item \alert{Momentum}
    \item \alert{Nesterov accelerated gradient}
    \item \alert{RSMprop}
    \item \alert{Adam}
\end{itemize}
\end{frame}
...
El \alert{learning rate decay} se basa en la idea de que el \alert{learning rate} comience con valores altos y se vaya \alert{reduciendo} a lo largo del entrenamiento. De esta manera se consigue primero hacer una \alert{aproximación global} para posteriormente realizar una \alert{búsqueda local}.

El learning rate se calcula de la siguiente manera:
\begin{equation}
    \alert{\alpha = \alpha_0 r^{\lfloor \frac{i}{n} \rfloor}}
\end{equation}
donde \alert{$i$} es la \alert{iteración} del entrenamiento y \alert{$r$} (decay rate) y \alert{$n$} (decay steps) son hiperparámetros que controlan la función.
\end{frame}

\begin{frame}{Otros optimizadores}
\begin{figure}
\centering
    \includegraphics[width=0.7\textwidth]{Slides/figures/Tema 1/Optimizers_Formula.png}
    \caption{\cite{Optimizers_formula}}
\end{figure}
\end{frame}
...
\begin{figure}
\centering
    \includegraphics[width=0.8\textwidth]{Slides/figures/Tema 3/BiasVariance_1.jpg}
